import {dictionaries,normalizeBaziDisplayLocale,type BaziDisplayLocale} from "./dictionaries";
...
export function displayCode(value:unknown,locale:BaziDisplayLocale|string="zh"):string{
  if(value===undefined||value===null||value==="")return "—";
  if(typeof value==="boolean")return displayCode(String(value),locale);
  ...
  const raw=String(value),normalized=normalizeBaziDisplayLocale(locale),local=dictionaries[normalized];
  return local[raw]||local[raw.toLowerCase()]||dictionaries.en[raw]||dictionaries.en[raw.toLowerCase()]||raw;
}
export function hasDisplayTranslation(code:string,locale:BaziDisplayLocale|string="zh"):boolean{const dictionary=dictionaries[normalizeBaziDisplayLocale(locale)];return Boolean(dictionary[code]||dictionary[code.toLowerCase()]);}
