...
export default async function LocaleLayout({ children, params }: { children: React.ReactNode; params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-[var(--line)]" style={{ background: "rgba(237,232,224,.82)", backdropFilter: "saturate(160%) blur(14px)" }}>
        <div className="mx-auto flex h-[70px] max-w-[var(--maxw)] items-center justify-between px-7">
          <Link href={`/${locale}`} className="flex items-center gap-2.5 font-[var(--serif)] text-[21px] font-semibold tracking-[.3px] text-[var(--ink)]">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--ink)] font-sans text-[17px] font-semibold" style={{ color: "var(--bg)" }}>八</span>
            FateLumen
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <ThemeSwitcher />
            <Link href={`/${locale}/login`} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[var(--line)] px-4 text-[13px] font-medium text-[var(--ink-soft)] hover:border-[var(--ink-faint)] hover:bg-[var(--bg-card)] transition-all">{t("signIn")}</Link>
          </div>
        </div>
      </header>
      <main className="relative z-[2]">{children}</main>
      <footer className="border-t border-[var(--line)] py-14 px-0 pb-11">
