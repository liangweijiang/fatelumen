...
var chapterRegistry = []ChapterDefinition{
	chapter(1, "destiny_depth", "命格深析", "解释日主强弱、五行喜忌、十神结构、四柱宫位与关键应期。", []string{"chart", "day_master_strength"}, []string{"chart", "element_strength", "day_master_strength", "ten_god_structure", "stem_relations", "branch_relations", "pattern_candidates", "favorable_elements", "luck_cycles", "annual_fortunes"}, sections("day_master_strength:日主强弱深层拆解", "element_logic:五行生克与喜忌逻辑", "ten_gods_mapping:十神组合与人生映射", "pillars_palaces:四柱宫位详解", "luck_analysis:大运推演", "key_periods:关键应期")),
	chapter(2, "ten_gods_full", "十神全览", "逐项解释十神的出现、力量、位置及现实含义。", []string{"chart", "ten_god_structure"}, []string{"chart", "day_master_strength", "ten_god_structure", "stem_relations", "branch_relations"}, sections("bi_jian:比肩", "jie_cai:劫财", "shi_shen:食神", "shang_guan:伤官", "zheng_cai:正财", "pian_cai:偏财", "zheng_guan:正官", "qi_sha:七杀", "zheng_yin:正印", "pian_yin:偏印")),
	chapter(3, "luck_cycle", "大运走势", "解释起运、当前大运、下一步大运、换运节点和人生节奏。", []string{"chart", "luck_cycles"}, []string{"chart", "day_master_strength", "favorable_elements", "luck_cycles", "annual_fortunes"}, sections("overview:起运与一生大运总览", "current:当前大运深度解读", "next:下一步大运展望", "transition:换运节点提醒", "advice:核心建议")),
	chapter(4, "ten_year_years", "未来十年流年", "严格解释后端给出的十年流年及月份干支，不自行推算。", []string{"chart", "luck_cycles", "annual_fortunes"}, []string{"chart", "day_master_strength", "ten_god_structure", "stem_relations", "branch_relations", "favorable_elements", "luck_cycles", "annual_fortunes"}, sections("overview:总运", "career_wealth:事业财运", "love_family:感情家庭", "health:健康", "key_months:关键月份", "advice:建议")),
	chapter(5, "career_depth", "事业深析", "解释事业模式、职业方向、职场关系、阶段机会与风险。", []string{"chart", "ten_god_structure"}, []string{"chart", "day_master_strength", "ten_god_structure", "pattern_candidates", "favorable_elements", "luck_cycles", "annual_fortunes"}, sections("overview:事业格局定性", "direction:职业方向指引", "wealth_logic:求财方式与逻辑", "key_periods:事业关键节点", "workplace:职场环境与人事", "entrepreneurship:创业与合伙时机", "risks:风险与避坑指南")),
	chapter(6, "wealth_depth", "财富格局", "解释财富结构、求财路径、现金流习惯和方向性风险管理。", []string{"chart", "ten_god_structure"}, []string{"chart", "day_master_strength", "ten_god_structure", "pattern_candidates", "favorable_elements", "luck_cycles", "annual_fortunes"}, sections("overview:财富总量定性", "wealth_stars:财星状态拆解", "path:求财方式与路径", "retention:财富流动与守成能力", "turning_points:财富转折点", "risks:破财风险与避坑", "allocation:财富配置建议")),
	chapter(7, "love_depth", "情感姻缘", "解释夫妻宫、配偶星、关系模式、阶段信号和沟通建议。", []string{"chart", "branch_relations"}, []string{"chart", "ten_god_structure", "stem_relations", "branch_relations", "luck_cycles", "annual_fortunes"}, sections("overview:情感格局定性", "partner:配偶画像", "spouse_palace:夫妻宫深度拆解", "timeline:感情时间线与关键节点", "relationship:相处模式与经营建议", "risks:风险与避坑指南", "current:当前感情指引")),
	chapter(8, "health_depth", "健康养生", "基于寒热燥湿和五行偏颇提供非诊断性的生活方式建议。", []string{"element_strength", "climate"}, []string{"chart", "element_strength", "climate", "disease", "mediation", "favorable_elements", "luck_cycles"}, sections("constitution:先天体质定性", "elements_organs:五行与五脏对应拆解", "risk_cycles:大运与健康风险期", "annual_health:流年健康应期", "daily:日常养生方向", "emotion:情绪与精神健康", "stage:阶段性养生重点")),
	chapter(9, "element_tuning", "五行调候", "解释寒暖湿燥、调候需求、环境方向和大运变化。", []string{"element_strength", "climate", "useful_god"}, []string{"chart", "element_strength", "climate", "disease", "mediation", "useful_god", "favorable_elements", "luck_cycles"}, sections("overview:调候定性", "climate_detail:寒暖湿燥拆解", "useful_element:调候用神分析", "health:调候对健康的影响", "emotion:调候对性格与情绪的影响", "environment:环境与方位调候", "luck_changes:大运调候变化")),
	chapter(10, "life_plan", "人生规划", "汇总阶段战略、关键窗口、风险地图与近期行动优先级。", []string{"chart", "luck_cycles", "annual_fortunes"}, []string{"chart", "element_strength", "day_master_strength", "ten_god_structure", "pattern_candidates", "climate", "useful_god", "favorable_elements", "luck_cycles", "annual_fortunes"}, sections("overview:人生总纲", "stages:分阶段战略", "windows:关键窗口期", "risks:风险地图", "career_wealth:财富与事业主线", "love_family:感情与家庭主线", "health:健康主线", "annual_action:年度行动纲领")),
}

func chapter(no int, key, name, purpose string, required, defaults []string, items []SectionDefinition) ChapterDefinition {
	return ChapterDefinition{no, key, name, purpose, required, defaults, items, chapterTemplatesZH[key]}
}
